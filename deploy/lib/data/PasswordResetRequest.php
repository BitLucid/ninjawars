...
class PasswordResetRequest extends Model{
    ...
    **/
    public function isActive(){
		...
		$id = PasswordResetRequest::select('id')->where('created_at', '>', $fourHours)->where('request_id', '=', $this->id())->lists('id');
		$is_used = (bool) $this->used;
		return ($is_used && (bool) $id);
    }
	...
	**/ 
	private static function sendResetNotification($email){
		$body = '
	...
	**/
	public static function match($token){
		$request = PasswordResetRequest::first(['nonce'=>$token]);
		if($request->isActive()){
			return $request;
		} else {
			return null;
	...
	/**
	 * Generate a full password reset request for an account id.
	**/
	public static function generate($account_id, $nonce=null){
		$nonce = $nonce !== null? $nonce : nonce();
		return PasswordResetRequest::create(['_account_id'=>$account_id, 'nonce'=>$nonce]);
	}
