...
abstract class BaseSettings extends BaseObject implements Persistent
{
     ...
     * Peer class name
     */
    const PEER = 'deploy\\model\\SettingsPeer';
    ...
    /**
     * The value for the settings_store field.
     * @var        string
     */
    ...
    /**
     * Get the [settings_store] column value.
     *
     * @return string
     */
    public function getSettingsStore()
    {

        return $this->settings_store;
    }
    ...
    public function setPlayerId($v)
    {
        ...
        if ($this->player_id !== $v) {
            $this->player_id = $v;
            $this->modifiedColumns[] = SettingsPeer::PLAYER_ID;
        }
    ...
    /**
     * Set the value of [settings_store] column.
     *
     * @param  string $v new value
     * @return Settings The current object (for fluent API support)
     */
    public function setSettingsStore($v)
    {
        if ($v !== null && is_numeric($v)) {
            $v = (string) $v;
        }

        if ($this->settings_store !== $v) {
            $this->settings_store = $v;
            $this->modifiedColumns[] = SettingsPeer::SETTINGS_STORE;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            $this->settings_store = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
            $this->resetModified();
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if (null === $this->setting_id) {
            try {
                $stmt = $con->query("SELECT nextval('settings_setting_id_seq')");
                $row = $stmt->fetch(PDO::FETCH_NUM);
                $this->setting_id = $row[0];
            } catch (Exception $e) {
        ...
        if ($this->isColumnModified(SettingsPeer::PLAYER_ID)) {
            $modifiedColumns[':p' . $index++]  = '"player_id"';
        }
        if ($this->isColumnModified(SettingsPeer::SETTINGS_STORE)) {
            $modifiedColumns[':p' . $index++]  = '"settings_store"';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '"player_id"':
                        ...
                        break;
                    case '"settings_store"':
                        $stmt->bindValue($identifier, $this->settings_store, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    {
        if (isset($alreadyDumpedObjects['Settings'][serialize($this->getPrimaryKey())])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Settings'][serialize($this->getPrimaryKey())] = true;
        $keys = SettingsPeer::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getSettingId(),
            $keys[1] => $this->getPlayerId(),
            $keys[2] => $this->getSettingsStore(),
        );
    ...
    public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) $this->setSettingsStore($arr[$keys[2]]);
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(SettingsPeer::PLAYER_ID)) $criteria->add(SettingsPeer::PLAYER_ID, $this->player_id);
        if ($this->isColumnModified(SettingsPeer::SETTINGS_STORE)) $criteria->add(SettingsPeer::SETTINGS_STORE, $this->settings_store);
    ...
    public function buildPkeyCriteria()
    {
        ...
        $criteria->add(SettingsPeer::PLAYER_ID, $this->player_id);
    ...
    public function isPrimaryKeyNull()
    {
        ...
        return (null === $this->getSettingId()) && (null === $this->getPlayerId());
    }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setSettingsStore($this->getSettingsStore());
        if ($makeNew) {
            $copyObj->setNew(true);
