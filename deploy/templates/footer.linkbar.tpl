		<div id='footer-middle-bar' style='padding:1.75rem;'>
			<div id='created-by' style='margin-top:1rem'>
				<a href='https://bitlucid.com' target='_blank'>brought to you by BitLucid, Inc.</a>
			</div>
			<ul id='footer-authors'>
				<li class='author'>
					<img class='avatar' alt='' src="https://www.gravatar.com/avatar/68dd1255208cbf50f2c42615bbbd8f46?d=monsterid&amp;80&amp;r=x">
					<a href='https://royronalds.com' class='extLink'>Roy Ronalds</a>
					<a href='/player?player=tchalvak'>Ninja: Tchalvak</a>
					<a href='https://twitter.com/tchalvak' rel='nofollow' class='extLink'>@tchalvak</a>
				</li>
				<li class='author'>
					<img class='avatar' alt="" src="https://www.gravatar.com/avatar/01b8df4923c0559d3ff56e6922e35011?d=monsterid&amp;80&amp;r=x">
					<a style='cursor:pointer;text-decoration:none'>Al Vazquez</a>
					<a href='/player?player=beagle'>Ninja: Beagle</a>
				</li>
			</ul>
        </div>
        <div id='footer-bottom-bar'>
        	<span id='html5-integration'>
				<div id='latest-commit-section'>
					<div style='padding:1rem'>
						<p id='latest-commit-title' style='display:none;font-size:smaller'>
							UPCOMING CHANGES
						</p>
						<div id='latest-commit' style='display:none'>
						</div>
					</div>
				</div>
			</span>
        	<!-- Will load commit info to the page -->
        	<script type='text/javascript' src="{cachebust file="/js/repo.js"}"></script>
		</div>
