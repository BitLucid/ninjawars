...
<div class='usage-mod-result'>
  ...
  {if $kill}
	You have killed {$targetObj->name()|escape} with {$article|escape} {$itemName|escape}!<br>
	You receive {$loot|escape} gold from {$targetObj->name()|escape}.<br>
  ...
  {if not $selfTarget}
    {include file='defender_health.tpl' health=$targetHealth level=$targetObj->level target_name=$targetObj->name()}
  {/if}
  ...
  {if $repeat}
  <br>
  <a class='attack-again thick btn btn-primary' href="/item/use/{$itemType|escape:'url'|escape}/{$targetObj->id()|escape:'url'|escape}/?link_back={$return_to|escape:'url'|escape}{if $selfTarget}&amp;selfTarget=1{/if}">
...
{if $return_to eq 'player'}
  <a href="/player?player_id={$targetObj->id()|escape:'url'}" class='return-to-location'>{if isset($charName)}view {$charName}{else}view ninja{/if}</a>
